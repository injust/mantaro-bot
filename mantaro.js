...
client.on("message", (msg) => {
	if (msg.author.id === config.bots.mantaro.id && (msg.content[0] === "✖" || msg.embeds.length > 0 && msg.embeds[0].author.name === "Trivia Game") || msg.content.includes("'s inventory:** ")) {
...
const sweepUser = async (channel, worker, primary) => {
	...
	if (messages[channel] != null && msg.content.includes("'s inventory:** ")) {
...
const run = async (channel, worker, primary) => {
	for (let i = 0;; i++) {
		...
		if (messages[channel] != null && messages[channel][0] === "✖") {
		...
		await sleep(config.intervals.send);
		for (let j = 0; j < 5; j++) {
			if (messages[channel] != null && messages[channel].embeds.length > 0 && messages[channel].embeds[0].author.name === "Trivia Game") {
